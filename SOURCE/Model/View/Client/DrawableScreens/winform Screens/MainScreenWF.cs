    ...
    public partial class MainScreenWF : Form, IScreenControls
    {
        ...
        public MainScreenWF(bool loadplayership)
        {
            ...
            if (loadplayership && ChooseThisShipString != null)
            {
                try
                {
                    PlayerShipClass choose = loadXML.loadedPlayerShips[ChooseThisShipString];
            ...
            {
                IEnumerable<KeyValuePair<string, PlayerShipClass>> defaultPSC =
                    loadXML.loadedPlayerShips.Where(s => s.Value.DefaultLoad);
                if (defaultPSC.Count() >= 1)
                {
                    KeyValuePair<string, PlayerShipClass> dPSC = defaultPSC.First();
