...
namespace Project.View.Client.DrawableScreens.WPF_Screens
{
    public partial class CreateGambleGameWF : Form, IScreenControls
    {
        ...
        public void Init()
        {
            ...
            shipcounttrackbar.Maximum = (int)Math.Ceiling((double)PlayerCredits / (double)MinShipCost);
            CurrShipCount = 1;
        ...
        private void CopyLabels()
        {
            ...
            int basebonus = (int)(CurrShipCost * basemod);
            destroybonuslabel.Text = basebonus.ToString();

            var gambperc = Shared.mapRange(CurrShipCost, MinShipCost, MaxShipCost, 1f, 2f);
            float diff = CurrShipCount * (float)(gambperc);
            difficultylabel.Text = ((int)diff).ToString();

            int diffcap = (int)diff;
            if (diffcap > 10)
                diffcap = 10;
            if (diffcap < 1)
                diffcap = 1;

            float colourp = (float)Shared.mapRange(diffcap, 1, 10, 0, 1);
            float inv = (float)Shared.mapRange(diffcap, 1, 10, 1, 0);

            System.Drawing.Color c = Color.FromArgb((int)(255 * colourp), (int)(255 * inv), 0);
            ...
            int modbonus = (int)(basebonus * diff);
            difficultybonuslabel.Text = modbonus.ToString();
            var totalbonus = modbonus * CurrShipCount;
