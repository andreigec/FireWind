...
namespace Project.View.Client.DrawableScreens.WPF_Screens
{
    public static class ShopScreens
    {
        public static void LoadSelectedParts(ref List<IPurchasable> typelist, Type currentType, bool onlyOwned)
        {
            ...
            var ow = GameControlClient.playerShipClass.OwnedWeapons;
            var op = GameControlClient.playerShipClass.OwnedParts;
            var os = GameControlClient.playerShipClass.OwnedShipBases;

            if (currentType == typeof(weapon))
            {
                typelist.AddRange(loadXML.loadedWeapons.Where(xx => ow.Contains(xx.Value) == onlyOwned).Select(x => x.Value));
            }
            else if (currentType == typeof(ShipBatteryPart))
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipBatteryPart)).Select(x => x.Value));
            }
            else if (currentType == typeof(ShipBoosterPart))
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipBoosterPart)).Select(x => x.Value));
            }
            else if (currentType == typeof(ShipEnginePart))
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipEnginePart)).Select(x => x.Value));
            }
            else if (currentType == typeof(ShipGeneratorPart))
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipGeneratorPart)).Select(x => x.Value));
            }
            else if (currentType == typeof(ShipHullPart))
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipHullPart)).Select(x => x.Value));
            }
            ...
            {
                typelist.AddRange(loadXML.loadedShipParts.Where(s => op.Contains(s.Value) == onlyOwned && s.Value.GetType() == typeof(ShipWingsPart)).Select(x => x.Value));
            }
        }
        ...
        public static void ChangeCurrentType(ref IPurchasable currentPartEquip, SlotLocation.SlotLocationEnum sl)
        {
            ...
            else if (sl == SlotLocation.SlotLocationEnum.LeftWeapon)
            {
                if (GameControlClient.playerShipClass.PlayerShip.Slots.ContainsKey(sl))
                    currentPartEquip = GameControlClient.playerShipClass.PlayerShip.Slots[SlotLocation.SlotLocationEnum.LeftWeapon];
            }
            else if (sl == SlotLocation.SlotLocationEnum.RightWeapon)
            {
                if (GameControlClient.playerShipClass.PlayerShip.Slots.ContainsKey(sl))
                    currentPartEquip = GameControlClient.playerShipClass.PlayerShip.Slots[SlotLocation.SlotLocationEnum.RightWeapon];
            }
            else
            {
        ...
        {
            var el = SlotLocation.GetEnumStringList();
        ...
        public static void RotatePartLists(ref IPurchasable currentPart, ref IPurchasable equipped, List<IPurchasable> PartList, int offset)
        {
