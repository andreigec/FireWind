...
namespace Project.View.Client.DrawableScreens.WPF_Screens
{
    public partial class PurchasableItemWFControl : UserControl
    {
        ...
        /// </summary>
        private Dictionary<Type, List<String>> reverseAttributes;

        /// <summary>
        /// attributes which arent compared usefully
        /// </summary>
        private Dictionary<Type, List<String>> ignoreAttributes;
        ...
        public bool Init(IPurchasable buyPart, IPurchasable equippedPart, bool hideCosts = false)
        {
            ...
            var cred = GameControlClient.playerShipClass.Credits;
            if (hideCosts == false)
            {
        ...
        public void AddItemRow(ref int yvalue, FieldInfo f)
        {
            ...
            if (BuyPart != null && EquippedPart != null)
            {
                ...
                {
                    var v1 = bool.Parse(po.ToString());
                    var v2 = bool.Parse(eqo.ToString());
                    equipcompare = v2.CompareTo(v1);
                }

                //change attributes
                var pt = BuyPart.GetType();

                bool ignore = (ignoreAttributes.ContainsKey(pt) && ignoreAttributes[pt].Contains(f.Name));
                if (ignore)
                    equipcompare = 0;

                bool reverse = equipcompare != 0 && (reverseAttributes.ContainsKey(pt) && reverseAttributes[pt].Contains(f.Name));
                if (reverse)
                {
                    if (equipcompare == -1)
                        equipcompare = 1;
            ...
            if (eqo != null && po != null)
            {
                var badc = Color.DarkRed;
                var goodc = Color.DarkGreen;
                var okc = Color.Teal;
