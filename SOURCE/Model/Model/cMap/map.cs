...
namespace Project
{
	public partial class Map : SharedID, IshipAreaSynch
	{
        ...
        */
		...
		{
			UpdateModes um = parentGCS.UPDATEFLAGS;
			//update map 
		...
		private void updateBuildings(GameTime gameTime, UpdateModes um)
		{
			...
			{
				BuildingInstance b = buildings[a];
				...
				if (Shared.flagChecked(um, UpdateModes.CheckHitboxes))
				{
					...
					*/
					...
					if (Shared.flagChecked(um, UpdateModes.RemoveOutOfBounds) &&
						Collisions.isTerrainCollisionBlock(this, s.spriteInstance.move.Position.Middle, 15))
					{
		...
		public void RemoveShot(WeaponInstance wi)
		{
			...
			{
				WeaponInstance wip = GetShot(wi.previousBeamInstanceID);
				if (wip != null && wip.nextBeamInstanceID != -1)
				{
					WeaponInstance win = GetShot(wip.nextBeamInstanceID);
					if (win == wi)
					{
		...
		redo:
			for (int a = 0; a < shots.Count; a++)
			{
					...
					Shared.flagChecked(um, UpdateModes.UpdateTerrain))
				{
					...
					bool removebuildhit = CheckBuildingsRemoveShot(s, gameTime, um, false, false);
					bool removeshiphit = CheckShipsRemoveShot(s, gameTime, um, false, false);
					bool terrainhit = Collisions.isTerrainCollisionRadius(this, s.spriteInstance.move.Position.Middle,
																		  s.GetWeaponFrameSize());

					bool remove = removebuildhit || removeshiphit || terrainhit;
		...
		public BuildingInstance getSuitableHangarShipRelease()
		{
			...
			foreach (BuildingInstance bi in buildings)
			{
		...
		public VectorMove getSuitableJoinGameVector(ShipInstance si)
		{
			...
			while (col)
			{
				...
				{
					//get random pos
					int x = deadzoneMinX + 1 + r.Next() % (deadzoneMaxX - deadzoneMinX - 5);
